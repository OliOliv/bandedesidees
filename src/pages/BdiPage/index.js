...
class BdiPage extends Component {
  ...
  getOneEvent = async function () {
    const res = await fetch(
      path2 + `events/getoneevent/${this.props.eventName}`
    );
    if (res.status === 201) {
  ...
  getSpeakersOfEvent = async function () {
    const res = await fetch(
      path2 + `speakers/getspeakersofevent/${this.props.eventName}`
    );
    if (res.status === 201) {
      const data = await res.json();
  ...
  getBooksOfEvent = async function () {
    const res = await fetch(
      path2 + `books/getBooksOfEvent/${this.props.eventName}`
    );
    if (res.status === 201) {
      const data = await res.json();
  ...
  getPhotographyOfEvent = async function () {
    const res = await fetch(
      path2 + `photography/getPhotographyOfEvent/${this.props.eventName}`
    );
    if (res.status === 201) {
      const data = await res.json();
  ...
  getIllustrationsOfEvent = async function () {
    const res = await fetch(
      path2 + `illustrations/getIllustrationsOfEvent/${this.props.eventName}`
    );
    if (res.status === 201) {
      const data = await res.json();
